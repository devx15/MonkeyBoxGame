...
class Ball {
    ...
    draw() {
        ...
        drawCircle(this.coordinate.x, this.coordinate.y-15, 15, "img/monkey_sprite.png")
...
class Stick {           // cross stick is not now :(
    ...
    draw() {
        ...
        drawStick(this.x1, this.y1, this.x2, this.y2, 13, "img/branch.png")
    }
    getAsset() {
...
class Block {
    ...
    draw() {
        ...
        //drawRectangle(13 + this.index.x*20, 13 + this.index.y*11, 19, 10, "img/green_banana.png")
...
function loadLevel(index) {
    ...
    for (var i = 0 ; i < levels[index].length; i++) {
        let bananas = {
            0: "img/green_banana.png",
            1: "img/yellow_banana.png"
        }
        let num = Math.round(Math.random())
        new Block(levels[index][i].xInd, levels[index][i].yInd, bananas[num])
...
function getCoor(e){
    if(!paused) {
        x=e.clientX
        y=e.clientY
        let newX1 = movableStick.x1
        let newX2 = movableStick.x2
...
document.addEventListener('keydown', getCoor)

function getCoorMouse(e){
    if(!paused) {
        x=e.clientX
        y=e.clientY
...
function drawWelcome() {
    drawText("Monkey Bounce", "#d9effc", "50px Saira Stencil One", 20, 275)
}

function fixHighestScore(scoreNow) {
...
function drawGameOver() {
    ...
    fixHighestScore(score)
    drawText("Highest Score " + highestScore, "#ddd", "24px Noto Sans", 130, 400)
...
function drawBoard() {
    ...
    fixHighestScore(score)
    drawText("Highest Score " + highestScore, "#ddd", "16px Noto Sans", 30, 20)
